...
const Resturentfoodlist = () => {
  ...
  return (
    <div className='food--list'>
      <button className='resturentadd' style={{ marginTop: '20px' }}>
        <div></div>
        <a href='/admin'>Add Restaurant</a>
      </button>
      <button
        className='resturentadd'
        style={{ marginTop: '20px' }}
        onClick={() => fetchFoodList(resturentid)}
      >
        <div></div>
        Fetch Food List
      </button>
              ...
              <li key={index} className='list-containers'>
                <div className='lists'>
                  <div className='food--details'>
                    ...
                    <h2 style={{ fontSize: 20, fontWeight: 600, marginTop: 10 }}>
                      {food.name}
                    </h2>
      ...
      {showPopup && (
        <div className='popup'>
          <div className='popup-inner'>
            <h2 style={{ color: 'pink', fontSize: '25px' }}>
              Upload Image for {selectedFood.name}
            </h2>
            <input type='file' onChange={handleFileUpload} />
            <button
              onClick={handleClosePopup}
              style={{ backgroundColor: '#526d82', borderRadius: '10px', padding: '10px' }}
            >
              Close
            </button>
          </div>
